...
from gamma_afsd import domain_mgr, heat_solve_mgr
...
a1 = 10000
young1 = cp.array(np.loadtxt('../0_properties/Al2024_Young.txt')[:,1]/1e6)
temp_young1 = cp.array(np.loadtxt('../0_properties/Al2024_Young.txt')[:,0])
Y1 = cp.array(np.loadtxt('../0_properties/Al2024_Yield.txt')[:,1]/1e6*np.sqrt(2/3))
temp_Y1 = cp.array(np.loadtxt('../0_properties/Al2024_Yield.txt')[:,0])
scl1 = cp.array(np.loadtxt('../0_properties/Al2024_Alpha.txt')[:,1])
temp_scl1 = cp.array(np.loadtxt('../0_properties/Al2024_Alpha.txt')[:,0])
...
idirich = cp.array(domain.nodes[:, 2] == -5.0 )
...
last_mech_time = 0
output_timestep = 1
filename = 'results/thinwall_{}.vtk'.format(file_num)
save_vtk(filename)
file_num = file_num + 1

start_time = time.time()
start_time_clock = time.strftime('%H:%M:%S', time.localtime(start_time))
...
while domain.current_time<endtime-domain.dt:
    ...
    if domain.current_time >= last_mech_time + implicit_timestep:
        ...
        K_elast,B,D_elast,ele_B,ele_D,iD,jD,ele_detJac, vD = elastic_stiff_matrix(active_eles,active_nodes,domain.Bip_ele, shear[0:n_e_active], bulk[0:n_e_active], batch_size=10000)
...
end_time = time.time()
end_time_clock = time.strftime('%H:%M:%S', time.localtime(end_time))
print('Start time: ', start_time_clock)
print('End time: ', end_time_clock)
simulation_time = end_time - start_time
print('Simulation time: ', simulation_time)

output = (
    f"Start Time: {start_time_clock}\n"
    f"End Time: {end_time_clock}\n"
    f"Simulation Time: {simulation_time:.2f} seconds\n"
)
