...
def elastic_stiff_matrix(elements, nodes, Bip_ele, shear, bulk, batch_size=10000):
    n_n = nodes.shape[0]
    ...
    Jac = cp.matmul(Bip_ele, nodes_pos[:, cp.newaxis, :, :].repeat(8, axis=1))
    ele_detJac = cp.linalg.det(Jac)
    iJac = cp.linalg.inv(Jac)
    ele_gradN = cp.matmul(iJac, Bip_ele)
    ...
    # Assemble D_elast in CSR format
    temp = cp.arange(0, n_int * 6).reshape(n_e, n_q, 6)
    iD = temp[:,:,cp.newaxis,:].repeat(6, axis=2)
    jD = temp[:,:,:,cp.newaxis].repeat(6, axis=3)
    vD = ele_D

    D_elast = cusparse.csr_matrix(
        (cp.ndarray.flatten(vD), (cp.ndarray.flatten(iD), cp.ndarray.flatten(jD))),
        shape=(6 * n_int, 6 * n_int), dtype=cp.float_
    )
    ...
    B = cusparse.csr_matrix(
        (cp.ndarray.flatten(vB), (cp.ndarray.flatten(iB), cp.ndarray.flatten(jB))),
        shape=(6 * n_int, 3 * n_n), dtype=cp.float_
    )

    # Fast GPU-based spgemm in batches
    from cupyx.scipy.sparse import csr_matrix

    row_chunks = list(range(0, D_elast.shape[0], batch_size)) + [D_elast.shape[0]]
    K = csr_matrix((n_dofs, n_dofs), dtype=cp.float_)

    for i in range(len(row_chunks) - 1):
        r0, r1 = row_chunks[i], row_chunks[i+1]
        D_block = D_elast[r0:r1, :]
        B_block = B[r0:r1, :]
        K += B_block.T @ (D_block @ B)

    return K, B, D_elast, ele_B, ele_D, iD, jD, ele_detJac, vD
