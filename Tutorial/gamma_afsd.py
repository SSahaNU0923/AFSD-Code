...
def load_toolpath(filename = 'toolpath_100.crs'):
...
class domain_mgr():
    ...
    def load_file(self):
        ...
        #         # option *SET_NODE_LIST
                ...
                #Modified for AFSD
                elif line.split()[0] == '*P_Rs_Rp_w_v_mu_slip':
                    line = next(f)
                    text = line.split()
                    self.pressure = float(text[0])  # Normal stress applied to the pin, MPa
                    self.r_shoulder = float(text[1])  # Shoulder radius, mm
                    self.r_pin = float(text[2]) # Pin radius, mm
                    self.w = float(text[3]) # Rotational speed, rad/s
                    self.v = float(text[4]) # Traverse speed, mm/s
                    self.mu = float(text[5])  # Friction coefficient
                    self.slip = float(text[6])  # Slip coefficient
                    ...
                    while True:
                        ...
                        if line.split()[1] == 'convection_2' or line.split()[1] == 'Convection_2' :
                            line = next(f)
                            self.h_conv_2 = float(line.split()[2])
        ...
        element_birth = np.array(birth_list_element, dtype=np.float64)
    ...
    def generate_surf(self):
        elements = self.elements.astype(np.int64)
        nodes = self.nodes.get().astype(np.int64)
        element_birth = self.element_birth.astype(np.int64)
        
        ele_num = elements.shape[0]
        connElEl = -np.ones([ele_num,100],dtype=np.int64)
        connVec = elements.flatten()
        conn_to_el_Vec = np.arange(0,ele_num,dtype=np.int64)[:,np.newaxis].repeat(8,axis=1).flatten()
        connVecIndx = np.arange(0,ele_num*8,dtype=np.int64)
    ...
    def get_timestep(self):
        ...
        for i in range(len(self.mat_thermal)):
            ...
            if self.mat_thermal[i][6] == -1:
                max_Cond = self.thermal_TD[i+1][1][:,1].max()
            else:
            ...
            print("Critical timestep: ", self.dt)
...
class heat_solve_mgr():
    def __init__(self,domain):
        ...
        self.r_shoulder = domain.r_shoulder
        self.r_pin = domain.r_pin
        self.w = domain.w
        self.v = domain.v
        self.mu = domain.mu
        self.slip = domain.slip
        
        self.h_conv = domain.h_conv
        self.h_conv_2 = domain.h_conv_2 
    ...
    def update_fluxes(self):
        ...
        r2 = cp.square(cp.linalg.norm(ip_pos-tool_center,axis=2))
        r = cp.sqrt(r2)

        q1 = (self.mu * self.pressure * self.w * r + self.mu * self.pressure * self.v)/1000
        q2 = (self.slip * self.mu *self.pressure * self.w * r)/1000 +  ((1 - self.slip) * self.pressure * self.w * r / cp.sqrt(3))/1000

        qmov = cp.where(r < self.r_pin, q1, cp.where(r < self.r_shoulder, q2, 0)) * surface_xy * laser_state
    ...
    def time_integration(self):
        ...
        # Update birth elements and related conditions
        domain.update_birth()
        self.update_cp_cond()
        self.update_mvec_stifness()

        # Update tool center and laser state from toolpath
        self.tool_center = domain.toolpath[self.current_step, 0:3]
        self.laser_state = domain.toolpath[self.current_step, 3]
        self.update_fluxes()

        # Explicit temperature update based on heat equation
        self.temperature[domain.active_nodes] += ( domain.dt * self.rhs[domain.active_nodes] / self.m_vec[domain.active_nodes])

        # Apply convective boundary condition at the bottom surface
        if self.isothermal == 1:
            bottom_nodes = cp.where(domain.nodes[:, 2] == -self.height)[0]
            self.rhs[bottom_nodes] -= self.h_conv_2 * (self.temperature[bottom_nodes] - self.ambient)

        # Recompute temperature at bottom nodes after applying convection
        self.temperature[bottom_nodes] += (domain.dt * (-self.h_conv_2 * (self.temperature[bottom_nodes] - self.ambient)) / self.m_vec[bottom_nodes])
